const musicList = (slug, img, title, author) => {
  return `
        <div data-slug="${slug}" id="balal" class="border border-amber-300 shrink-0 relative group">
...
const musicListCol = (slug, img, title, author) => {
  return `
        <div data-slug="${slug}" id="balal" class="flex border border-amber-300 shrink-0 relative group w-[300px]">
            <img class="h-[100px] mr-[10px]" src=${img}/>
            <div id"anh"  class=" absolute hover:cursor-pointer group-hover:block hidden bg-[linear-gradient(rgba(0,0,0,0.502),rgba(0,0,0,0),rgba(0,0,0,0))]  w-[100%] top-0 right-[66%] bottom-0"></div>
            <div>
